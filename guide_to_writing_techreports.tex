...
as in \SI{120}{\celsius}. However, for angles, there is no space before the degree (90\si{\degree}). The BIPM \href{https://www.bipm.org/en/publications/si-brochure/section5-3-7.html}{treats \% as a unit}, so it should also be preceded by a space: \SI{2.1}{\%}. For ranges, use an en dash (30--\SI{50}{\celsius}); for intervals one may state the unit only once at the end (30, 40, and \SI{50}{\celsius}).
...
For litre, only use L or mL to avoid the risk of confusing l (lower case ell) with 1 (one) or I (upper case i). Note that SI allows the use of l, but not $\ell$.

Remember to \emph{be consistent} when you have made an optional choice like using a dot instead of a space for multiplication.
...
\subsection{Figures}
Figures are centred in the text with no border around it. Use similar fonts (both size and typeface) in the figure as in the main text. Figures containing photographs should be processed to be legible in black and white, including labels and the legend. Box the legend to distinguish its markers from the chart, unless it is very large, in which case the legend is placed next to the chart area.

A caption is shown below the figure and always ensure that it explains the figure in more detail than ``A plot of height over time''---this is visible from the graph.
If the caption fits on one line, it should be centre-justified.
If it does not, it should be left or full justified with a hanging indent as shown in Figure~\ref{fig:figexample}. If the caption is a sentence fragment, no period is used. But if any complete sentence occurs in a caption, then all sentences, and any sentence fragments, in that caption should end with a period.
...
\item Do use suitable axes for showing the information you need to be seen (logarithmic for exponential behaviour, \textit{etc}.).
...
\begin{itemize}
\item Don't fit a spline (smooth curve) through the data points (this indicates an exact solution or analytical model). 
